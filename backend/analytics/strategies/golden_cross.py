...
def run(df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
    """
    Golden cross strategy: SMA50 crossing above SMA200.

    Returns the most recent golden cross signal, if any.
    """
    if df.empty:
    ...
    # Golden cross where SMA50 crosses above SMA200
    ...
    if events.empty:
        return {"message": "No golden cross detected", "has_signal": False}
